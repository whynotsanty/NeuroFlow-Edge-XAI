...
def train_one_fold(model, train_loader, val_loader, device, epochs, fold_idx, save_path, current_best_f1):
    criterion = nn.CrossEntropyLoss()
    # AdamW introduzido para maior rigor de regularização em dados sensíveis
    ...
    for epoch in range(epochs):
        ...
        # Avaliação empírica a cada época para evitar guardar um modelo em overfitting
        metrics = evaluate_fold(model, val_loader, device)
        epoch_f1 = metrics["macro_f1"]
        
        print(f"  Época [{epoch + 1:02d}/{epochs}] | Train Loss: {mean_loss:.4f} | Val F1: {epoch_f1:.4f}")

        # Guarda o modelo APENAS se bater o recorde global
        if epoch_f1 > current_best_f1:
            current_best_f1 = epoch_f1
            torch.save(model.state_dict(), save_path)
            print(f"    [!] Novo checkpoint guardado (Macro F1: {current_best_f1:.4f})")

    return current_best_f1, metrics
...
def train_and_profile(model_name=MODEL_NAME, epochs=NUM_EPOCHS):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(split_indices, start=1):
        ...
        # Passamos o val_loader e o current_best_f1 para gerir os checkpoints rigorosamente
        global_best_f1, final_metrics = train_one_fold(
            model, train_loader, val_loader, device, epochs, fold_idx, model_save_path, global_best_f1
        )
    ...
    if not os.path.exists(model_save_path):
        raise RuntimeError("Falha grave: O modelo nunca foi guardado. Verifica se as métricas estão a ser calculadas corretamente.")
    ...
    # Medir a latência do modelo final que foi efetivamente guardado
    ...
    print(f"Macro F1 médio global: {mean_macro_f1 * 100:.2f}%")
    print(f"Melhor Macro F1 absoluto (Guardado): {global_best_f1 * 100:.2f}%")
